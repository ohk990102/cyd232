  ...
  while ((!CHECK_IF_1(u)) && (!CHECK_IF_1(v))) {
    ...
    while (CHECK_IF_EVEN(v)) {
      ...
      if (CHECK_IF_EVEN(x2)) {
        ...
        __ECC_BN_add(&x2, p, &x2);
        ECC_BN_rshift_1bit(&x2, &x2);
      }
    }

    if (ECC_BN_cmp(&u, &v) >= 0) {
      ECC_BN_sub_mod(&u, &u, &v, p);
      ECC_BN_sub_mod(&x1, &x1, &x2, p);
    }
    else {
      ECC_BN_sub_mod(&v, &v, &u, p);
...
int ECC_point_add(ECC_POINT* R, ECC_POINT* P, ECC_POINT* Q)
{
  ...
  if (!ECC_BN_cmp(&P->x, &Q->x))
  {
    ...
    ECC_BN_binary_inv(&d, &d, &prime_p256);
...
int ECC_ECDSA_sign(ECC_BN* r, ECC_BN* s, ECC_BN* hm, ECC_BN* k, ECC_BN* d)
{
	ECC_POINT a;
	ECC_point_init(&a);
	ECC_point_smul(&a, k, &base_p256);
	ECC_BN t1, t2, t3;
	ECC_BN_shift_sub_mod(&t1, &a.x, &order_p256);
	if (t1.len == 0)
		return ECC_FAIL;
	ECC_BN_binary_inv(&t2, k, &order_p256);
	ECC_BN_mul_mod(&t3, d, &t1, &order_p256);
	ECC_BN_add_mod(&t3, hm, &t3, &order_p256);
	ECC_BN_mul_mod(&t2, &t2, &t3, &order_p256);
	if (t2.len == 0)
		return ECC_FAIL;
	ECC_BN_copy(r, &t1);
	ECC_BN_copy(s, &t2);
...
int ECC_ECDSA_verification(ECC_BN* r, ECC_BN* s, ECC_BN* hm, ECC_POINT* Q)
{
	ECC_BN w, u1, u2, v;
	//1. Verify that r and s are integers in the interval[1, n −1].If any verification fails
	//   then return(“Reject the signature”).
	if (ECC_BN_cmp(r, &order_p256) >= 0)
		return ECC_FAIL;
	if (ECC_BN_cmp(s, &order_p256) >= 0)
		return ECC_FAIL;

	//2. Compute e = H(m).
	//3. Compute w = s−1 mod n.
	ECC_BN_binary_inv(&w, s, &order_p256);
	//ECC_BN_shift_sub_mod(&t1, &t1, &order_p256);
	...
	ECC_point_init(&t1);
	ECC_point_init(&t2);
	ECC_point_smul(&t1, &u1, &base_p256);
	ECC_point_smul(&t2, &u2, Q);
	ECC_point_add(&t1, &t1, &t2);

	//6. If X = ∞ then return(“Reject the signature”);
	if (t1.point_at_infinity == 1)
		return ECC_FAIL;

	//7. Convert the x - coordinate x1 of X to an integer x1; compute v = x 1 mod n.
	ECC_BN_shift_sub_mod(&v, &t1.x, &order_p256);
